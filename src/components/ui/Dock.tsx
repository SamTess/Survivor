...
type DockItemProps = {
  className?: string;
  children: React.ReactElement<HoverableChildProps> | React.ReactElement<HoverableChildProps>[];
  onClick?: () => void;
  mouseX: MotionValue;
  spring: SpringOptions;
  distance: number;
...
function DockItem({
  ...
  return (
    ...
    >
      {Children.map(
        children,
        child => cloneElement(child as React.ReactElement<HoverableChildProps>, { isHovered })
      )}
    </motion.div>
  );
}
